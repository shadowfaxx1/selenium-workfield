...
def initialization():
    ...
    stopword_folder=r"C:\Users\kaifk\OneDrive\Desktop\all important docx\StopWords"  #folder not file change the folder to file and change the stopwords file opening function 
    dictionary_postive=r"C:\Users\kaifk\OneDrive\Desktop\all important docx\MasterDictionary\positivewords.txt"
    dictionary_negative=r"C:\Users\kaifk\OneDrive\Desktop\all important docx\MasterDictionary\negativewords.txt"
...
def write_into_csv(x):
    ...
    with open(r"C:\Users\kaifk\OneDrive\Desktop\all important docx\output.csv", 'w',encoding='utf-8', newline='' ) as fp:
        wr= csv.writer(fp)
        wr.writerow(columns)  
        wr.writerows(x)
    fp.close()
...
def gathering_info(id,url):
    with open(id,'r',encoding='utf-8') as fp: 
        text=fp.read()
    fp.close()
    cleaned_file=cleaning_file(text)   #this return a cleaned file without stopwords
    return sentiment_analysis(cleaned_file,url ) #this further sents the file to get all stated values 
